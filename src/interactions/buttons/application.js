...
async function handleReview(interaction, isApprove) {
    ...
    try {
        ...
        return interaction.reply({ embeds: [errorEmbed('No Permission', 'You need the **Manage Server** permission or a configured manager role to review applications.')], flags: ['Ephemeral'] });
    }

    const application = await getApplication(interaction.client, interaction.guild.id, appId);
    if (!application) return interaction.reply({ embeds: [errorEmbed('Application not found. It may have been deleted.')], flags: ['Ephemeral'] });

    if (application.status !== 'pending') {
        const { emoji, label } = statusDisplay(application.status);
        return interaction.reply({ embeds: [errorEmbed('Already Reviewed', `This application has already been marked as **${label}** ${emoji}`)], flags: ['Ephemeral'] });
    }

    const action = isApprove ? 'Approve' : 'Deny';
    const modalId = `app_reason:${appId}:${isApprove ? '1' : '0'}`;

    const modal = new ModalBuilder().setCustomId(modalId).setTitle(`${isApprove ? '✅' : '❌'} ${action} Application`);
    modal.addComponents(
        new ActionRowBuilder().addComponents(
            new TextInputBuilder()
                ...
                .setPlaceholder(isApprove ? 'Welcome message or notes...' : 'Why is this being denied?')
                .setMaxLength(500)
                .setRequired(false),
        ),
    );
    ...
    let reasonSubmit;
    try {
        reasonSubmit = await interaction.awaitModalSubmit({ time: 5 * 60 * 1000, filter: i => i.customId === modalId && i.user.id === interaction.user.id });
    } catch { return; }

    await reasonSubmit.deferReply({ flags: ['Ephemeral'] });

    const reason = reasonSubmit.fields.getTextInputValue('reason').trim() || 'No reason provided.';
    const status = isApprove ? 'approved' : 'denied';
    const { emoji, label } = statusDisplay(status);
    const statusColor = isApprove ? (getColor('success') || '#57F287') : (getColor('error') || '#ED4245');

    try {
        await ApplicationService.reviewApplication(reasonSubmit.client, interaction.guild.id, appId, {
            action: isApprove ? 'approve' : 'deny', reason, reviewerId: interaction.user.id,
        });

        // Update the log embed — remove buttons, update status field, add reviewer note
        try {
            ...
            if (logMsg.embeds.length > 0) {
                ...
                const updatedEmbed = EmbedBuilder.from(logMsg.embeds[0])
        ...
        if (isApprove) {
            ...
            } catch (err) { logger.error('Failed to assign role', { error: err.message }); }
        }
        ...
        try {
            ...
            const dmEmbed = new EmbedBuilder()
                ...
                .setDescription(`Your application for **${application.roleName}** has been **${status}**.\n\n**Note from staff:** ${reason}`)
                .setColor(statusColor)
                .setTimestamp();
            if (isApprove) dmEmbed.addFields({ name: '🎉 Next Steps', value: `You've been given the role in **${interaction.guild.name}**. Welcome!`, inline: false });
            await user.send({ embeds: [dmEmbed] });
        } catch { logger.warn('Could not DM applicant', { userId: application.userId }); }

        await reasonSubmit.editReply({
            embeds: [
                new EmbedBuilder()
                    .setTitle(`${emoji} Application ${label}`)
                    .setDescription(`You have **${status}** the application from <@${application.userId}> for **${application.roleName}**.\n\n**Reason:** ${reason}`)
                    .setColor(statusColor),
            ],
        });
    } catch (err) {
        logger.error('Error processing application review', { error: err.message, appId, stack: err.stack });
        await reasonSubmit.editReply({ embeds: [errorEmbed('Review Failed', 'An error occurred. Please try again.')] });
    }
}
